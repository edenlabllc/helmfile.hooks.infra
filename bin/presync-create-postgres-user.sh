...
function create_default_user() {
  for DB in ${PG_DB_NAMES[*]}; do
    ...
    if ! (kubectl get secrets -n "${NAMESPACE}" | grep "^${DEFAULT_OWNER_USER//_/-}\.${PG_CLUSTER_NAME}" &> /dev/null); then
...
function create_custom_user() {
  if ! (kubectl get secrets -n "${NAMESPACE}" | grep "^${NAMESPACE}\.${PG_DB_USERNAME//_/-}\.${PG_CLUSTER_NAME}" &> /dev/null); then
    kubectl patch "${PG_CRD_NAME}" "${PG_CLUSTER_NAME}" -n "${PG_NAMESPACE}" --type='merge' \
